<template>
  ...
  <div>
    <app-header>
      ...
      <div slot="title">找回密码</div>
      <div slot="right"></div>
    </app-header>
    <div class="box">
      <div :class="flag1 ? 'init-num active' : 'init-num'">
        <input
          type="text"
          placeholder="请输入手机号"
          maxlength="11"
      ...
      <div :class="flag2 ? 'init-num active' : 'init-num'">
        <input
          type="text"
          placeholder="请输入验证码"
          maxlength="11"
      ...
      <div :class="flag3 ? 'init-num active' : 'init-num'">
        <input
          type="password"
          placeholder="请输入密码"
          maxlength="11"
...
export default {
  ...
  methods: {
    ...
    async ok(){
        ...
        let {data:res}=await this.$http.okuppwd(obj)
        if(res.code==200){
            Toast("修改密码成功")
