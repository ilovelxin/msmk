  ...
  <div class="content">
    <div class="page-login">
      <div class="login-content">
        ...
        <div :class="active_flag ? 'active init-num' : 'init-num'">
          <input
            style="width: 53.33333vw"
            type="text"
            placeholder="请输入手机号"
            maxlength="11"
            @focus="active_flag = true"
            v-model="mobile"
            @blur="active_flag = false"
          />
          <div @click="sms">获取验证码</div>
        </div>
        <div :class="active_flags ? 'active init-num' : 'init-num'">
          <input
            type="text"
            placeholder="请输入短信验证码"
            minlength="6"
            maxlength="18"
            @focus="active_flags = true"
            @blur="active_flags = false"
            v-model="sms_code"
          />
        </div>
        <div class="others">
          <span>*未注册的手机号将自动注册</span>
          <span @click="$router.push('/login')">密码登录</span>
        </div>
      </div>
      <div class="login">
...
export default {
  ...
  methods: {
    ...
    async sms() {
      ...
      let { data: res } = await this.$http.sms(obj);
      if (res.code == 200) {
        Toast("短信发送成功请注意查收短信");
        return false;
      }
